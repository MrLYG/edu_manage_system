    ...
    >
      ...
      <div class="inner" slot-scope="{ data, node }">
        ...
        <span class="actions">
          ...
          <el-button v-if="node.level == 1" size="small" @click.stop="handleShowAddLesson(data)">+添加课时</el-button>
...
<!-- 添加或修改课时 -->
    <el-dialog class="add-dialog" title="课时信息" :visible.sync="showAddLesson">
      ...
      >
        ...
        <el-form label-position="right" label-width="70px" :model="isFreeForm">
        <el-form-item label="是否开放试听">
          <el-select @change="$forceUpdate()" v-model="isFreeForm.is_free_status" >
            <el-option
              v-for="(item,index) in Object.keys(isFreeMapping)"
              :key="index"
              :label="isFreeMapping[item]"
              :value="item"
            ></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleAddLesson(){
      ...
      this.addLessonForm.is_free = this.isFreeForm.is_free_status
      console.log(this.addLessonForm)
      axios.post("/courseContent",{
        methodName: "saveOrUpdateLesson", 
        lesson: this.addLessonForm
      }).then(resp=>{
        console.log("handleAddLesson成功！")
        this.showAddLesson = false
        return this.loadChildren(this.addSectionForm.course_id)
      })
      .catch(error => {
        this.showAddSection = false
        this.$message.error("handleAddLesson操作执行失败! ! !");
      });
    },
    //方法9: 修改课时
    handleEditLesson(data){
      ...
      axios.get("/courseContent",{
        params:{
          methodName:"getSectionNameById",
          section_id: data.section_id
        }
      }).then(resp=>{
        this.addLessonForm.section_name = resp.data.section_name
      })
      this.showAddLesson = true
    },
